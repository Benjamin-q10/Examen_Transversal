...
def stock_marca(stock, marca):
    ...
    for modelo, datos in productos.items():
        if datos[0].lower() == marca.lower():
            ...
            encontrados.append((modelo, cantidad))
    if encontrados:
...
def busqueda_precio(p_min,p_max):
    ...
    for modelo, datos in productos.items():
        ...
        if p_min <= precio <= p_max:
            encontrados.append((modelo, datos[0], precio))
    
    if encontrados:
        print("Productos encontrados:")
        for modelo, marca, precio in encontrados:
            print(f"Modelo: {modelo}, Marca: {marca}, Precio: {precio}")
    else:
